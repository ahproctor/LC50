...
d.lc50 <- with(summary(fit),
               cbind(lc50,
                     toxicity[match(rownames(lc50),toxicity$group),
...
estimates of LC50 can be obtained with `lc50JAGS`.  This function is
an analog of `lc50` that constructs an object of class `jags` that can
be used to sample from the posterior for the model using the
facilities of the **rjags** package.

To generate Bayesian estimates for the reduced model, `lc50JAGS` is
used to generate a model object
```{r}
library(rjags)
model <- lc50JAGS(cbind(alive,dead)~salinity+temperature,concentration=conc,group=group,data=toxicity)
```
...
any of the model variables "alpha", "beta", "gamma", "lc50",
"loglc50", "p" or "q" as required.
```{r}
s <- coda.samples(model,c("beta","q","lc50"),n.iter=10000,n.thin=10)
summary(s)
```
...
With shrinkage,
```{r}
fit <- lc50(cbind(alive,dead)~salinity+temperature,concentration=conc,group=group,data=toxicity,rate.shrink=0.001)
summary(fit,rate=TRUE)
```
