...
```{r}
fit <- lc50(cbind(alive,dead)~salinity*temperature,concentration=conc,group=group,data=toxicity,common.background = TRUE)
summary(fit)
```
...
```{r}
fit <- lc50(cbind(alive,dead)~salinity+temperature,concentration=conc,group=group,data=toxicity,common.background=T)
summary(fit)
```
...
summary(mc)
```
Multplicity adjusted confidence intervals for
